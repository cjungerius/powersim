...
fm1_null = fit(MixedModel, formula_zero, d)
...
function single_run(filename = nothing, args...; kwargs...)
  ...
  # the p-values calculated by MixedModels are uncorrected and therefore inflated - we use the best alternative method available to us, the likelihood ratio test vs the intercept-only model, to find a better estimate (note that we leave the intercept p value since it isn't of interest here)
